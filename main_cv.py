...
loss_fun = {
    'asl_loss': AsymmetricLossOptimized(gamma_neg=2, gamma_pos=1),
...
def train_one_fold(model, optimizer, scheduler, train_dataloader, val_dataloader, loss_dic, device, logger,
                   args, fold_dir=''):
    best_score = 0
    best_model_path = ''
...
def inference(model_list, dataloader, device, logger, post_transforms):
...
def main(args):
    ...
    if args.phase == 'train':
        ...
        for fold_i, (train_index, val_index) in enumerate(kfold.split(list(range(400)))):
            # create dataloader
            train_dataloader, val_dataloader, post_transforms = create_dataset(args, train_index, val_index)
            # create model and optimizer
            model, optimizer, scheduler = create_model(device, args)
            # cross validation
            best_model_path, best_score = train_one_fold(model, optimizer, scheduler, train_dataloader, val_dataloader,
                                                         loss_dic, device, logger, args, fold_dir=f'fold_{fold_i}')
        ...
        inference(model_list, test_dataloader, device, logger, post_transforms)
